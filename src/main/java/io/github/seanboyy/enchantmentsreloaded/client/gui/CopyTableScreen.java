...
import io.github.seanboyy.enchantmentsreloaded.network.packet.PacketItemCopiedRequest;
...
@OnlyIn(Dist.CLIENT)
public class CopyTableScreen extends ContainerScreen<CopyTableContainer> {
    private static final ResourceLocation COPY_TABLE_GUI_TEXTURE = new ResourceLocation(EnchantmentsReloaded.MOD_ID, "textures/gui/container/copytable.png");
    ...
    public CopyTableScreen(CopyTableContainer screenContainer, PlayerInventory inv, ITextComponent titleIn){
    ...
    protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        this.minecraft.getTextureManager().bindTexture(COPY_TABLE_GUI_TEXTURE);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if(d0 >= 0 && d1 >= 0 && d0 < BUTTON_WIDTH && d1 < BUTTON_HEIGHT) {
            if(this.container.copyItem(this.minecraft.player, this.container.getCopyableItem())) {
                Network.sendPacketToServer(new PacketItemCopiedRequest(this.container.getCopyableItem()));
                return true;
            }
        }
